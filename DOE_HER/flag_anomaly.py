...
class DetectTrajAnomaly:
    def __init__(self, init_atoms, final_atoms, atoms_tag):
        ...
        self.any_anomalies = self.has_any_anomalies()

    def has_any_anomalies(self):
        """
        True if any anomalies are present
        """
        adsorbate_dissociated = self.is_adsorbate_dissociated()
        adsorbate_desorbed = self.is_adsorbate_desorbed()
        surface_reconstructed = self.is_surface_reconstructed()
        return any([adsorbate_dissociated, adsorbate_desorbed, surface_reconstructed])
    ...
    def is_adsorbate_desorbed(self, neighbor_thres=3):
        ...
        for idx in adsorbate_atoms_idx:
            ...
            surface_neighbors = [
                n.index for n in neighbors if n.index in surface_atoms_idx
            ]
